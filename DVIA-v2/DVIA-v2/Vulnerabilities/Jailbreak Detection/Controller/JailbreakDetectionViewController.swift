...
//        [56, 61, 50, 32, 32, 101, 89, 29, 94, 32, 19, 27, 37, 2, 9, 74, 4, 14, 25, 75, 73, 88, 47, 62, 63, 14, 15, 75, 19, 21, 79, 75, 47, 52, 54]
    ...
    @IBAction func jailbreakTest4Tapped(_ sender: Any) {
        let isJailbroken = self.isJailbroken()
                ...
                if isJailbroken {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(5), execute: { exit(0) })
                } else {
...
#if !SIMULATOR
        // Now uses the improved Objective-C implementation that supports rootless
    ...
    @inline(__always) func jailbreakTest3() {
        let isJB = isJailbroken()   // now uses improved check
                
                let message = isJB ? "Device is Jailbroken, the application will now exit" : "Device is Not Jailbroken"
    ...
    @IBAction func jailbreakTest5Tapped(_ sender: Any) {
        let isJB = isJailbroken()   // Now uses the modern rootless-aware check
        
        if isJB {
            let alertController = UIAlertController(title: "",
                                                    message: "Oops, something went wrong, the application will now exit!",
            ...
            let alertController = UIAlertController(title: "",
                                                    message: "Device is Not Jailbroken",
                                                    preferredStyle: .alert)
            let action = UIAlertAction(title: "OK", style: .cancel, handler: nil)
            alertController.addAction(action)
            present(alertController, animated: true, completion: nil)
